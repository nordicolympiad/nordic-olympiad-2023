...
int main() {
  ...
  for (auto entry : adj_map) {
    ...
    for (auto edge : entry) {
      max_weight = max(max_weight, edge.second);
    }
  }
  ...
  for (int q = 0; q < qs; q++) {
    ...
    if (kgram_id.find(kgram_vec) == kgram_id.end()) {
      ...
      while (lo <= hi) {
        ...
        if (longest_path[kgram] >= w) {
          res = mid;
          lo = mid+1;
          best_next_vertex = vector<int>(next_vertex, next_vertex + (int)adj.size());
        } else {
          hi = mid-1;
        }
      }
